...
public:
    vector<vector<int>> verticalTraversal(TreeNode* root) {
        if (root == NULL)
            return {};
        offset = 0, rightmost = -1;
        findBounds(root, 0);
        vector<vector<int>> result(rightmost + offset + 1);
        vector<pair<TreeNode *, int>> layer;
        layer.push_back(make_pair(root, 0 + offset));
        while (layer.empty() == false) {
            ...
            for (auto p : layer) {
                ...
                int idx = p.second;
                m_[idx].push_back(node->val);
                if (node->left)
                    next.push_back(make_pair(node->left, idx - 1));
                if (node->right)
                    next.push_back(make_pair(node->right, idx + 1));
            }
            for (auto &e : m_) {
                vector<int> &tmp = e.second;
                sort(tmp.begin(), tmp.end());
                result[e.first].insert(end(result[e.first]), tmp.begin(), tmp.end());
            }
            layer.swap(next);
        }
        return result;
...
private:
    int offset;
    int rightmost;
    void findBounds(TreeNode *root, int X) {
        if (root == NULL)
            return;
        findBounds(root->left, X - 1);
        if (X + offset < 0)
            offset = -X;
        findBounds(root->right, X + 1);
        if (rightmost < X)
            rightmost = X;
        return;
    }
