...
public:
    ...
    void dfs(vector<vector<int>> &image, int i, int j, int src, int newColor) {
        image[i][j] = newColor;
        if (i > 0 && image[i-1][j] == src)
            dfs(image, i - 1, j, src, newColor);
        if (j > 0 && image[i][j-1] == src)
            dfs(image, i, j - 1, src, newColor);
        if (i < image.size() - 1 && image[i+1][j] == src)
            dfs(image, i + 1, j, src, newColor);
        if (j < image[0].size() - 1 && image[i][j+1] == src)
            dfs(image, i, j + 1, src, newColor);
        return;
    }
};
