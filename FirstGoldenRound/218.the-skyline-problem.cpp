 ...
 * 
 */
class CP { // for Critical Point    
public:
    int x;
    int h;
    int isLeft;
    CP(int _x, int _h, int _isLeft) : x(_x), h(_h), isLeft(_isLeft) {}
    bool operator< (CP other) { return x == other.x ? h < other.h : x < other.x; };
};
class Solution {
public:
    vector<vector<int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        for (auto b : buildings) {
            points.push_back(CP(b[0], b[2], true));
            points.push_back(CP(b[1], b[2], false));
        }
        sort(points.begin(), points.end());

        priority_queue<pair<int, bool>> ontop;
        vector<vector<int>> result;
        function<void(void)> poppair = [&]() {
            auto top = ontop.top();
            if (top.second == false)
                return;
            ontop.pop();
        ...
        for (auto point : points) {
            if (point.isLeft) {
                int preTop = ontop.empty() ? 0 : ontop.top().first;
                ontop.push(make_pair(point.h, false));
                int curTop = ontop.top().first;
                if (curTop > preTop) {
                    if (!result.empty() && point.x == result.back()[0])
                        result.pop_back();
                    if (result.empty() || point.h != result.back()[1])
                        result.push_back({point.x, point.h});
                }
            }
            else {
                int preTop = ontop.top().first;
                ontop.push(make_pair(point.h, true));
                poppair();
                int curTop = ontop.empty() ? 0 : ontop.top().first;
                if (curTop < preTop) {
                    result.push_back({point.x, curTop});
                }
            }
        }
